...
const GeographiePage: React.FC = () => {
  ...
  const encouragementMessages = [
    ...
    "🗺️ Excellent sens de l'orientation !",
    "🏔️ Continue d'explorer le monde !",
    "🌊 Tes connaissances géographiques s'améliorent !",
    "🏜️ Tu deviens un expert en géographie !",
  ...
  useEffect(() => {
    const fetchExercises = async () => {
      try {
        ...
        const savedUserAnswers = localStorage.getItem('geography_userAnswers');
        const savedResults = localStorage.getItem('geography_results');
        const savedValidatedExercises = localStorage.getItem('geography_validatedExercises');
        ...
        if (!savedUserAnswers || !savedResults || !savedValidatedExercises) {
          ...
          if (userId && token) {
            const answersResponse = await axios.get(
              `${process.env.NEXT_PUBLIC_API_URL}/answers/${userId}/geography`,
              {
                headers: {
            ...
            localStorage.setItem('geography_userAnswers', JSON.stringify(userAnswersMap));
            localStorage.setItem('geography_results', JSON.stringify(resultsMap));
            localStorage.setItem('geography_validatedExercises', JSON.stringify(validatedExercises));
  ...
  const handleSubmit = async (id: string, correctAnswer: string) => {
    ...
    if (exerciseIndex !== -1) {
      ...
      localStorage.setItem('geography_userAnswers', JSON.stringify(updatedUserAnswers));
      localStorage.setItem('geography_results', JSON.stringify(updatedResults));
      
      // Sauvegarder l'état de validation
      const validatedExercises = JSON.parse(localStorage.getItem('geography_validatedExercises') || '{}');
      ...
      if (isCorrect) {
        ...
        if (currentStreak >= 3) {
          ...
          toast.success("Bonne réponse ! Continue à explorer la géographie ! 🏔️");
        }
      } else {
        setCurrentStreak(0);
  ...
  const calculateFinalScore = async () => {
    try {
      ...
      if (scorePercentage >= 90) {
        ...
        toast.success("Très bon travail ! Ta connaissance du monde est impressionnante ! 🗺️");
      } else if (scorePercentage >= 50) {
        toast.success("Bon travail ! Continue à explorer la géographie ! 🏔️");
      } else {
