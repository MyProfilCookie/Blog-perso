...
const GeographyPage: React.FC = () => {
  ...
  const encouragementMessages = [
    ...
    "🏔️ Continue d'explorer la Terre !",
  ...
  const handleSubmit = (id: string, correctAnswer: string) => {
    ...
    if (isCorrect) {
      ...
      if (currentStreak >= 3) {
        ...
        toast.success("Bonne réponse ! Continue à explorer le monde ! 🏔️");
      }
    } else {
      setCurrentStreak(0);
      // Messages d'encouragement pour les mauvaises réponses
      toast.error("Ce n'est pas la bonne réponse, mais ne t'inquiète pas ! Continue d'explorer ! 🌊");
    }
  };
  ...
  const calculateFinalScore = async () => {
    try {
      ...
      if (scorePercentage >= 90) {
        toast.success("Excellent travail ! Tu es un véritable explorateur ! 🌍");
      } else if (scorePercentage >= 70) {
        toast.success("Très bon travail ! Ta connaissance du monde est impressionnante ! 🗺️");
      } else if (scorePercentage >= 50) {
        toast.success("Bon travail ! Continue à découvrir notre planète ! 🏔️");
      } else {
        toast.info("Ne te décourage pas ! Le monde est vaste et plein de découvertes ! 🌊");
      }
