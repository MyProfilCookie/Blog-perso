...
const LanguagePage: React.FC = () => {
  ...
  const encouragementMessages = [
    "🌍 Tu es un excellent linguiste !",
    "🗣️ Ta maîtrise des langues est impressionnante !",
    "📚 Continue à explorer les langues !",
  ...
  useEffect(() => {
    const fetchExercises = async () => {
      try {
        ...
        const savedResults = localStorage.getItem('language_results');
        const savedValidatedExercises = localStorage.getItem('language_validatedExercises');
        ...
        if (!savedUserAnswers || !savedResults || !savedValidatedExercises) {
          ...
          if (userId && token) {
            const answersResponse = await axios.get(
              `${process.env.NEXT_PUBLIC_API_URL}/answers/${userId}/language`,
              {
                headers: {
            ...
            localStorage.setItem('language_userAnswers', JSON.stringify(userAnswersMap));
            localStorage.setItem('language_results', JSON.stringify(resultsMap));
            localStorage.setItem('language_validatedExercises', JSON.stringify(validatedExercises));
  ...
  const handleSubmit = async (id: string, correctAnswer: string) => {
    ...
    if (exerciseIndex !== -1) {
      ...
      localStorage.setItem('language_userAnswers', JSON.stringify(updatedUserAnswers));
      localStorage.setItem('language_results', JSON.stringify(updatedResults));
      
      // Sauvegarder l'état de validation
      const validatedExercises = JSON.parse(localStorage.getItem('language_validatedExercises') || '{}');
      ...
      if (isCorrect) {
        ...
        setCurrentStreak(0);
        toast.error("Ce n'est pas la bonne réponse, mais les langues s'apprennent en pratiquant ! Essaie encore ! 🎯");
      }
    }
  };
  ...
  const calculateFinalScore = async () => {
    try {
      ...
      if (scorePercentage >= 90) {
        toast.success("Excellent travail ! Tu es un véritable linguiste ! 🌍");
        ...
        toast.info("Ne te décourage pas ! Les langues sont un domaine passionnant à découvrir ! 🎯");
      }
