...
const HistoirePage: React.FC = () => {
    ...
    const fetchExercises = async () => {
      try {
        ...
        if (qList.length <= 3 || unique.size <= 3) {
          const fb = await loadFallbackExercises("history");
          if (fb.length) setExercises(fb);
        }

        // Charger l'historique des réponses seulement si pas de données dans le localStorage
        const savedUserAnswers = localStorage.getItem('history_userAnswers');
        const savedResults = localStorage.getItem('history_results');
        const savedValidatedExercises = localStorage.getItem('history_validatedExercises');
        ...
        if (!savedUserAnswers || !savedResults || !savedValidatedExercises) {
          ...
          if (userId && token) {
            const answersResponse = await axios.get(
              `${process.env.NEXT_PUBLIC_API_URL}/answers/${userId}/history`,
              {
                headers: {
            ...
            localStorage.setItem('history_userAnswers', JSON.stringify(userAnswersMap));
            localStorage.setItem('history_results', JSON.stringify(resultsMap));
            localStorage.setItem('history_validatedExercises', JSON.stringify(validatedExercises));
  ...
  const handleSubmit = async (id: string, correctAnswer: string) => {
    ...
    if (exerciseIndex !== -1) {
      ...
      localStorage.setItem('history_userAnswers', JSON.stringify(updatedUserAnswers));
      localStorage.setItem('history_results', JSON.stringify(updatedResults));
      
      // Sauvegarder l'état de validation
      const validatedExercises = JSON.parse(localStorage.getItem('history_validatedExercises') || '{}');
      ...
      if (isCorrect) {
        ...
        setCurrentStreak(0);
        toast.error("Ce n'est pas la bonne réponse, mais l'histoire est faite d'apprentissage ! Essaie encore ! 🗡️");
      }
    }
  };
  ...
  const calculateFinalScore = async () => {
    try {
      ...
      if (scorePercentage >= 90) {
        ...
        toast.success("Bon travail ! Continue à explorer l'histoire ! ⚔️");
      } else {
